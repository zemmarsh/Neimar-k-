...
# --- 1. Настройка путей для PyInstaller и FFmpeg ---
def get_bin_path(bin_name: str) -> str:
    """Возвращает путь к исполняемому файлу (ffmpeg/ffprobe) внутри .exe или в проекте."""
...
# Получаем локальные пути к бинарникам
FFMPEG_BIN = get_bin_path("ffmpeg.exe")
FFPROBE_BIN = get_bin_path("ffprobe.exe")

# Добавляем папку с бинарниками в системный PATH текущего процесса Python
bin_dir = os.path.dirname(FFMPEG_BIN)
os.environ["PATH"] = bin_dir + os.pathsep + os.environ.get("PATH", "")

# --- 2. Настройка логирования в папку logs ---
...
# --- 4. Проверки и аналитика ---
def check_ffmpeg() -> bool:
    """Проверяет доступность ffmpeg по прямому пути или из PATH."""
    try:
        cmd = [FFMPEG_BIN if os.path.exists(FFMPEG_BIN) else "ffmpeg", "-version"]
        subprocess.run(
            cmd,
...
def get_media_duration(file_path: str) -> float:
    ...
    try:
        ffprobe_cmd = FFPROBE_BIN if os.path.exists(FFPROBE_BIN) else "ffprobe"
        cmd = [
            ffprobe_cmd,
            "-v",
            "error",
...
# --- 5. Конвертация медиа и изображений ---
...
# --- 6. Пакетная параллельная обработка ---
